...
class Setting extends MY_Controller 
{
	public function index()
	{
		...
		$this->load->view('setting', $out);
	}
	
	public function voucher()
	{
		$this->load->view('voucher/voucher_data');
	}
	
	public function voucher_json()
	{
		$this->load->model('m_barang');
		$level 			= $this->session->userdata('ap_level');

		$requestData	= $_REQUEST;
		$fetch			= $this->m_barang->fetch_data_voucher($requestData['search']['value'], $requestData['order'][0]['column'], $requestData['order'][0]['dir'], $requestData['start'], $requestData['length']);
		...
		{ 
			...
			{
				$nestedData[]	= "<a href='".site_url('barang/edit_voucher/'.$row['id_barang'])."' id='EditBarang'><i class='fa fa-pencil'></i> Edit</a>";
	...
	public function tambah_voucher()
	{
		...
		if($level == 'admin' OR $level == 'inventory')
		{
			if($_POST)
			{
				...
				{
					$this->form_validation->set_rules('kode['.$no.']','Kode Barang #'.($no + 1),'trim|required|max_length[40]|callback_exist_kode[kode['.$no.']]');
				...
				if($this->form_validation->run() == TRUE)
				{
					...
					foreach($_POST['kode'] as $k)
					{
						...
						$insert = $this->m_barang->tambah_baru($kode, $nama, $id_kategori_barang,'0' , $id_merk_barang, $stok, $harga, $keterangan, $type);
						if($insert){
							$inserted++;
						}
						$no_array++;
			...
			else
			{
				...
				$this->load->view('voucher/voucher_tambah', $dt);
			}
		}
		else
		{
	...
	public function edit_voucher($id_barang = NULL)
	{
		if( ! empty($id_barang))
		{
			$level = $this->session->userdata('ap_level');
			if($level == 'admin' OR $level == 'inventory')
			{
				if($this->input->is_ajax_request())
				{
					...
					if($_POST)
					{
						...
						$this->form_validation->set_rules('kode_barang','Kode Barang','trim|required|max_length[40]'.$callback);
						$this->form_validation->set_rules('nama_barang','Nama Barang','trim|required|max_length[60]|alpha_numeric_spaces');
						//$this->form_validation->set_rules('id_kategori_barang','Kategori','trim|required');
						$this->form_validation->set_rules('id_merk_barang','Merek','trim');
						$this->form_validation->set_rules('total_stok','Stok','trim|required|numeric|max_length[10]|callback_cek_titik[total_stok]');
					...
					else
					{
						...
						$dt['kategori'] = $this->m_kategori_barang->get_all();
						$dt['merek'] 	= $this->m_merk_barang->get_all();
						$this->load->view('voucher/voucher_edit', $dt);
					}
				}
			}
		}
