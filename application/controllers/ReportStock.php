...
class ReportStock extends MY_Controller 
{
	...
	function load_chart_Detail(){
		...
		$data .= "<thead>						
						...
						<th style='width:30%;text-align: center;'>Outlet</th>
						<th style='width:10%;text-align: center;'>Tanggal</th>
						<th style='width:15%; text-align: center;'>Document No</th>
						<th style='width:15%; text-align: center;'>External Doc No</th>";
		if ($tipe == "Both")	{
			$data .= "<th style='width:10%;text-align: center;'>Qty</th>";
			$data .= "<th style='width:15%;text-align: center;'>Amount</th>";
		}
		else {
			$data .="<th style='width:25%;text-align: center;'>". $tipe ."</th>";
		}			
		$data .= "</tr>						
    				</thead><tbody>";
	...
	function load_chart(){
		...
		if ($this->session->userdata('ap_level') == "OWNER"){
			$data .= "<thead>
						<tr>
						<th style='text-align: center;'>No</th>
						<th style='text-align: center;'>Code</th>
						<th style='width: 30%; text-align: center;'>Nama Barang</th>
						<th style='text-align: center;'>Category Location</th>
						<th style='text-align: center;'>End Stock</th>
						</tr>											
    				</thead>";
		}else{
			$data .= "<thead>
						...
						<th style='width: 30%; text-align: center;' rowspan='2'>Nama Barang</th>
						<th style='text-align: center;' rowspan='2'>Category Location</th>
		...
		if ($this->session->userdata('ap_level') == "OWNER"){
			...
			if(!empty($detail->ItemStockQty)){
				foreach($detail->ItemStockQty as $so){				
					...
					$data .= '<td align="right"><a href="#" tipe="Qty" itemname="'.$so->Name.'" subtipe="InFree" code="'. $so->Code .'" onclick="ShowDetail(this);">'.str_replace(",", ".", number_format($so->InFree)). '</a></td>';
					...
					$data .= '<td align="right"><a href="#" tipe="Qty" itemname="'.$so->Name.'" subtipe="OutFree" code="'. $so->Code .'" onclick="ShowDetail(this);">'.str_replace(",", ".", number_format($so->OutFree)). '</a></td>';
					$data .= '<td align="right"><a href="#" tipe="Qty" itemname="'.$so->Name.'" subtipe="Akhir" code="'. $so->Code .'" onclick="ShowDetail(this);">'.str_replace(",", ".", number_format($so->Akhir)). '</a></td>';				
					$data .= "</tr>";
					$i++;
				}
