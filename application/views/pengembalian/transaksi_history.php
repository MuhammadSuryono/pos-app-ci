			...
			<div class='table-responsive'>
				...
				<table id="example88" class="table table-striped table-bordered">
                    ...
                    <tbody>
                        <?php 
							$i = 1;
							if(!empty($sales_order->ReturOrder)){
								foreach($sales_order->ReturOrder as $so){
									
									echo '<tr>';
									echo '<td align="center">'.$i.'.</td>';
									echo '<td>'.date('d-M-Y', strtotime($so->TransactionDate)).'</td>';
									
									echo '<td><a href="'.site_url('pengembalian/detail_transaksi/'.$so->OrderNo).'" id="LihatDetailTransaksi"><i class="fa fa-file-text-o fa-fw"></i> '.$so->OrderNo.'</a></td>';
