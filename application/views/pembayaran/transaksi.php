...
function getPointValue(){	
	var url2 = '<?php echo site_url('pembayaran/get_point_convertion');?>';
	$.ajax({
	url : url2,
	type : "POST",
	success:function(response){
		var dt_json = JSON.parse(response);
		var point = dt_json['data'];
		$.each(point, function( key, value ) {					 
			document.getElementById("idr_point").innerHTML = value.idr_point;
			document.getElementById("idr_convert").innerHTML = value.idr_convert;
		});
...
function checkpoint(id){
	var url = '<?php echo site_url('pembayaran/get_point');?>';

	if(id != ''){		
		var val = id;
		var _val = val.split('Þ');
		if (_val[3] != "011111111111"){
			$.ajax({
			data : {val : _val[3]},
			url : url,
			type : "POST",
			success:function(response){
				...
				var point = dt_json['data'];
				$.each(point, function( key, value ) {					 
					document.getElementById("point_pelanggan").innerHTML = value.point;
				});
...
function chg_byr(val, indexnya){
	...
	if(val > 0 || val !=''){
		...
		if (val == 'POINT'){
			...
			$('.payType_'+indexnya).find('option').remove().end();
			$('.payType_'+indexnya).append($('<option/>').attr("value", "POINT").text("POINT"));
		}
...
function bayarBaru(nominal){
	...
	var Baris = "<tr>";
		...
		Baris += "</select>";
		Baris +="</td>";
		Baris += "<td><input type='text' class='form-control' id='nama_bank' idx='"+ idx +"' name='nama_bank[]' onkeypress='return check_int(event)'></td>";
		Baris += "<td>";
		Baris += "<input type='text' class='form-control nilai_bayar' value='"+ nominal +"' id='nilai_bayar"+idx+"' name='nilai_bayar[]' onkeypress='return check_int(event)'>";
		Baris += "<span></span>";
		Baris += "</td>";
...
$(document).on('keyup', '#nama_bank', function(e){
	var point = $("#point_pelanggan");
	var idr_point = $("#idr_point");
	var idr_convert = $("#idr_convert");
	var charCode = e.which || e.keyCode;
	if (parseInt(point.text()) != 0 && parseInt(point.text()) >= parseInt($(this).val()) || $(this).val() == ""){
		if (parseInt(<?php echo intval($grnd_ttl) ;?>) >= parseInt(idr_convert.text()) ){	
			if (parseInt(<?php echo intval($grnd_ttl) ;?>) < (parseInt(idr_point.text()) * parseInt($(this).val()))){				
				if ($(this).val() != ""){
					alert("Point Amount is greater than total amount.");
				}					
			}else{
				var idxz = $(this).attr("idx");

				$(this).val(function(index, value) {		
					var vals = (parseInt(idr_point.text()) * parseInt($(this).val()));
					
					$("#nilai_bayar" + idxz).val(vals.toString().replace(/\D/g, "")
					.replace(/\B(?=(\d{3})+(?!\d))/g, "."));
			...
			if ($(this).val() != ""){
				alert("You don't have any total Amount.");
			}			
			return 0;
		}		
	}else{
		alert("You don't have any point.");
		return 0;
	}
	//hitung_bayar();
});
...
function SimpanTransaksi(){
	...
	FormData += "&idr_convert="+$('#idr_convert').text();	
	FormData += "&idr_point="+$('#idr_point').text();
