...
$(document).on('click', '#TampilkanData', function(e){				
	...
	function TampilkanProduct(){
		var location = $('#StoreId').select2("val");
		var html = '';
		var url = '<?php echo site_url('ReportOwner/loatTopTenCat');?>';
		//e.preventDefault();
	...
	function ShowDetail(e) {
		...
		var url = '<?php echo site_url('ReportOwner/load_detail_product_group');?>';
		$.ajax({
				data : {
	...
	function ShowDetailPayment(e) {
		var location = $('#StoreId').select2("val");
		document.getElementById("message").innerHTML = "";	
		var url = '<?php echo site_url('ReportOwner/load_detail_sales_payment');?>';
		$.ajax({
				data : {
					StartDate : $('#tanggal_dari').val(), 
					EndDate : $('#tanggal_sampai').val(), 
