...
<?php 
/**
	Update 31-01-2021 - KIMID: 
		- Disabled input manual discount
		- Enhance Discount Function when click item promo and when update qty
**/
	$id = '';
	...
	if(!empty($itemku) && empty($itemku->DiscountAdhoc)){
		foreach($itemku as $it){
			$id = '';
			$vatsts = '';
			$id = $it->No.'Þ'.$it->Description.'Þ'.$it->Inventory.'Þ'.$it->Sales_Price.'Þ'.$it->Sales_Discount.'Þ'.$it->Vat.'Þ'.explode('!', $it->Description)[1];
			if ($it->Vat == 'NOVAT'){
				$vatsts = 'btn-success';	
			}else{
				$vatsts = 'btn-danger';
			...
			echo '<div class="col-sm-12 list-product itm_prd" id="'.$id.'">
                    ...
                    <div class="caption">
                    	<p style="color:#FFF; font-size:16px; margin: 0 0 -3px;"><b>'.$a[0].'</b><span class="badge badge-primary badge-pill pull-right" style="font-size:16px">Rp. '.number_format($it->Sales_Price,2,',','.').'</span></p>
						 '.$b.'
					</div>
...
function list_cart_promo(myID, harga, qty_promo, discount, vatstat, nama){
	...
	if(cnt_item > 0){
		...
		$('#disc_ttl_dis'+myID).val(numberWithCommas(Number($('#disc_ttl_'+myID).val()) + discount));
		$('#disc_'+myID).val(discount);
	}else{
		var discount_coma = numberWithCommas(discount);
...
function list_cart(id){
	...
	if(cnt_item > 0){
		...
		baris +='<input type="hidden" class="pull-left" onkeyup="promo(\'' +_id[0]+ '\')" id="disc_ttl_'+_id[0]+'" name="disc_ttl[]" ids="'+vatstat+'" placeholder="Disc." style="margin-top:-5px;width:25%;text-align:right"/>';
		baris +='<input type="text" disabled id="disc_ttl_dis'+_id[0]+'" style="margin-top:-5px;width:25%;text-align:right"/>';
...
function HitungTotalBayar(id, qty, hrg, disc, vatstat, ispromo){
	...
	if(vatstat != 'NOVAT'){		
		...
		if(ispromo == 0){
			$('#disc_ttl_'+id).val(disc * qty);
			$('#disc_ttl_dis'+id).val(numberWithCommas(disc * qty));
		}
	...
	if(vatstat == 'NOVAT'){		
		...
		if(ispromo == 0){
			$('#disc_ttl_'+id).val(_discNP * qty);
			$('#disc_ttl_dis'+id).val(numberWithCommas(_discNP * qty));
		}
