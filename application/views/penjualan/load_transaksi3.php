		...
		success:function(response){
			if(response != ''){
				...
				$(head["line"]).each(function (index) {
					var disc_int = parseInt($(this).attr('DiscountAmount').replace(',', ''));
					var price_int = parseInt($(this).attr('SalesPrice').replace(',', ''));
...
function list_cart_promo(myID, harga, qty_promo, discount, vatstat, nama){
  console.log(discount);
	...
	if(cnt_item > 0){
		var qty = Number($('#qty_'+myID).val()) + 1;
		$('#qty_'+myID).val(qty);
		$('#disc_ttl_'+myID).val(discount * qty);
	}else{
    ...
    baris +='<input type="hidden" class="_disc" name="_disc[]" id="disc_'+myID+'" value="'+discount+'" style="width:40%; text-align:center;" placeholder="Disc." disabled /></span>';
		baris +='<input type="text" class="pull-left" onkeyup="promo(\'' +myID+ '\')"  value="'+discount+'" id="disc_ttl_'+myID+'" name="disc_ttl[]" ids="'+vatstat+'" placeholder="Disc." style="margin-top:-5px;width:25%;text-align:right"/>';
...
function list_cart(id){
	...
	if(cnt_item > 0){
		...
		baris +='<input type="text" class="pull-left" onkeyup="promo(\'' +_id[0]+ '\')" id="disc_ttl_'+_id[0]+'" name="disc_ttl[]" ids="'+vatstat+'" placeholder="Disc." style="margin-top:-5px;width:25%;text-align:right"/>';
